...
public sealed class AudioTests : IDisposable
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "GondwanaGsnd-" + Guid.NewGuid());
        ...
        var gsndPath = Path.Combine(definitionDirectory, "audio.gsnd");
        ...
        try
        {
            ...
            AudioDefinitionSerializer.Save(gsndPath, manager);

            var definition = AudioDefinitionSerializer.Load(gsndPath);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void EngineStateAudioRoundTripsThroughInlineOrExternalGsnd(bool separateGsndFile)
    {
        var root = Path.Combine(Path.GetTempPath(), "GondwanaGsnd-" + Guid.NewGuid());
        var mediaDirectory = Path.Combine(root, "media");
        Directory.CreateDirectory(mediaDirectory);
        ...
        try
        {
            ...
            new EngineState().SaveToFile(
                ...
                separateGsndFile: separateGsndFile);

            var stateJson = File.ReadAllText(statePath);
            Assert.Contains("\"Audio\"", stateJson);

            if (separateGsndFile)
                Assert.True(File.Exists(Path.Combine(root, "state.audio", "audio.gsnd")));
