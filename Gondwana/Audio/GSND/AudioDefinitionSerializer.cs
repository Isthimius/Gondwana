...
/// <summary>
/// Provides loading, saving, conversion, and materialization helpers for Gondwana audio (.gsnd) files.
/// </summary>
public static class AudioDefinitionSerializer
{
    ...
    public static AudioDefinition Load(string filePath)
    {
        if (string.IsNullOrWhiteSpace(filePath))
            throw new ArgumentException("GSND file path must be a non-empty string.", nameof(filePath));

        var fullPath = Path.GetFullPath(filePath);
        if (!File.Exists(fullPath))
            throw new FileNotFoundException($"GSND file not found: {fullPath}", fullPath);
        ...
        {
            throw new InvalidDataException($"Failed to deserialize GSND file: {fullPath}", ex);
        }
    }
        ...
        string? baseDirectory = null)
    {
        ...
        if (errors.Count != 0)
        {
            throw new InvalidDataException(
                "GSND definition is invalid:" + Environment.NewLine +
        ...
        foreach (var resourceDefinition in definition.Resources)
        {
            ...
            AudioResource resource = resourceDefinition.SourceKind switch
            {
                ...
                _ => throw new InvalidDataException(
                    $"Unknown GSND source kind '{resourceDefinition.SourceKind}'.")
            };
        ...
        AudioResourceManager manager)
    {
        ...
        var assetsFile = AssetsFile.AllAssetsFiles.FirstOrDefault(
            ...
            ?? throw new InvalidDataException(
                $"GSND resource '{definition.Key}' references assets file '{assetsPath}', but it is not loaded.");

        using var stream = assetsFile.Get(AssetTypes.Audio, definition.AssetEntryName!)
            ?? throw new InvalidDataException(
                $"GSND resource '{definition.Key}' references missing audio entry '{definition.AssetEntryName}' in '{assetsPath}'.");
        ...
        string? sourceDescription)
    {
        if (string.IsNullOrWhiteSpace(json))
            throw new ArgumentException("GSND JSON content must be a non-empty string.", nameof(json));
        ...
        try
        {
            ...
            for (int i = 0; i < definition.Resources.Count; i++)
            {
                if (definition.Resources[i] is null)
                    throw new InvalidDataException($"GSND Resources cannot contain a null entry at index {i}.");
            }
    ...
    private static string? GetReferenceBaseDirectory(AudioDefinitionSource source)
    {
        return source.Kind switch
        {
            AudioDefinitionSourceKind.LooseDefinitionFile
                when !string.IsNullOrWhiteSpace(source.GsndFilePath) =>
                Path.GetDirectoryName(Path.GetFullPath(source.GsndFilePath)),
