...
public class HexagonalFlatTopCoordinates : ISceneLayerCoordinates
{
    public Point GetAnchorPixelAtSceneLayerCoordinates(SceneLayer sceneLayer, PointF gp)
    {
        ...
        int x = sceneLayer.ZeroPixel.X + (int)Math.Floor(col * (W * 0.75f));
        int y = sceneLayer.ZeroPixel.Y + row * H + ((col & 1) == 0 ? 0 : H / 2);
    ...
    {
        int W = sceneLayer.SceneLayerTileWidth; int H = sceneLayer.SceneLayerTileHeight;
        float fx = (pixelPt.X - sceneLayer.ZeroPixel.X) / (W * 0.75f);
        int approxCol = (int)Math.Round(fx);
        int baseY = sceneLayer.ZeroPixel.Y + ((approxCol & 1) == 0 ? 0 : H / 2);
        ...
        foreach (var cand in NeighborsFlatTop(approxCol, approxRow, includeSelf: true))
        {
            ...
            float cx = sceneLayer.ZeroPixel.X + cand.X * (W * 0.75f) + W / 2f;
            float cy = sceneLayer.ZeroPixel.Y + cand.Y * H + ((cand.X & 1) == 0 ? 0 : H / 2) + H / 2f;
    ...
    public List<SceneLayerTile> GetSceneLayerTilesInPixelRange(SceneLayer sceneLayer, Rectangle pixelRange, bool includeOverhang)
    {
        ...
        int minCol = (int)Math.Floor((pixelRange.Left - sceneLayer.ZeroPixel.X) / (W * 0.75f)) - 2;
        int maxCol = (int)Math.Ceiling((pixelRange.Right - sceneLayer.ZeroPixel.X) / (W * 0.75f)) + 2;

        for (int col = minCol; col <= maxCol; col++)
        {
            int yOffset = ((col & 1) == 0 ? 0 : H / 2);
            int minRow = (int)Math.Floor((pixelRange.Top - sceneLayer.ZeroPixel.Y - yOffset) / (float)H) - 2;
            int maxRow = (int)Math.Ceiling((pixelRange.Bottom - sceneLayer.ZeroPixel.Y - yOffset) / (float)H) + 2;
    ...
    private static Point[] HexPolygonFlatTop(SceneLayer sceneLayer, int col, int row, bool includeOverhang)
    {
        int W = sceneLayer.SceneLayerTileWidth; int H = sceneLayer.SceneLayerTileHeight;
        var p = new Point(
            sceneLayer.ZeroPixel.X + (int)Math.Floor(col * (W * 0.75f)),
            sceneLayer.ZeroPixel.Y + row * H + ((col & 1) == 0 ? 0 : H / 2));
