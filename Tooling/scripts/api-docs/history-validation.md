...
The first executing migration run failed before publication while decoding historical HTML as UTF-8. Its evidence records master `8cde2e9aeffc1454597c861abc8215f2dbfdce8d`, original Pages `4e0f5e2826a67d98210be61f9887e2061f071d57`, and verified backup `refs/heads/gh-pages-pre-versioning-20260909`. No rollback is needed for that run; retain both dated backup branches.

The runner used Doxygen **1.9.8**, not the locally tested **1.14.0**. Pinning 1.14.0 alone was tested on Linux and still reproduced invalid UTF-8 in the v2.1.0 `TextBlock` class page. The cause is platform-dependent encoding pattern matching: Doxygen 1.14.0's `adjustConfiguration()` lowercases `INPUT_FILE_ENCODING` patterns, while `genericPatternMatch()` leaves the actual mixed-case paths unchanged on a case-sensitive filesystem. See the pinned upstream [configuration implementation](https://github.com/doxygen/doxygen/blob/Release_1_14_0/src/doxygen.cpp) and [pattern matching](https://github.com/doxygen/doxygen/blob/Release_1_14_0/src/util.cpp).

The correction uses the supported input filter to transcode legacy C# text on stdout without changing tagged source bytes, and pins the official Linux x64 Doxygen 1.14.0 archive. SHA-256 `e5d6ae24d0bf3f0cdc4d8f146726b89ca323922f19441af99b1872d503665ad6` was verified against downloads from both the upstream GitHub release and doxygen.nl. All publishing workflows use the shared installer and run the real-Doxygen encoding regression after installation. Every build now strictly validates all HTML, reports the failed page and byte offset, and migration failures print the log tail directly in Actions.

The migration audit explicitly accepts the three new standard development assets (`clipboard.js`, `cookie.js`, `doxygen_crawl.html`) emitted by this Doxygen version, so ordinary Docs publication after merging the fix does not itself block the retry. Other unexpected files and symlinks still abort.

All 16 recorded releases and recorded master were successfully rebuilt on Linux using the fixed filter and official 1.14.0 executable. Complete-site validation passed: **293,517,057 bytes**, **20,731 files**, **zero invalid UTF-8 HTML pages**, all expected version labels/selector assets/provenance, and the stable/development manifest and redirects. Staged tree SHA-256: `8c80a0c9a919cf4d1cff5f4d4e4ff6ac86e9cb466c60786d5ef218a5d1c23898`. The post-merge migration audit also passed using the actual newly generated development files. All **19 Python tests** passed, including a real-Doxygen mixed-encoding regression and isolated Git backup/lease tests; shell and workflow YAML validation passed. Doxygen still emits nonfatal parser diagnostics (including `StateFileMount` scope handling); these are separate from the corrected UTF-8 failure and remain in the logs. The original Doxyfile and source files were not edited.
