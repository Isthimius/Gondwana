...
internal sealed class MainForm : Form
{
    ...
    private readonly List<SpriteEditorDocument> _documents = [];
    ...
    public MainForm()
    {
        ...
        foreach (var paneName in SpriteEditorControl.PaneNames)
        {
        ...
        _tree.NodeMouseDoubleClick += (_, e) =>
        {
            ...
            switch (Path.GetExtension(path).ToLowerInvariant())
            {
                ...
                case ".gts":
                    ...
                    break;
                case ".gscn":
                    EnsureEditor().AddSceneSources([path]);
                    break;
            }
        };
    ...
    private void ChooseDirectory()
    {
        ...
        {
            Description = "Choose the sprite working directory",
    ...
    private static bool IsAuthoringFile(string path)
    {
        ...
        return extension.Equals(".gspr", StringComparison.OrdinalIgnoreCase) ||
               extension.Equals(".gts", StringComparison.OrdinalIgnoreCase) ||
               extension.Equals(".gscn", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private void ChooseScenes()
    {
        ...
        if (dialog.ShowDialog(this) == DialogResult.OK)
            EnsureEditor().AddSceneSources(dialog.FileNames);
    }

    private SpriteEditorDocument EnsureEditor()
    {
    ...
    private void OpenDocument(string path)
    {
        try
        {
            ...
            ShowDocument(SpriteDocument.Open(path));
        }
        catch (Exception ex) when (
    ...
    private void NewDocument() =>
        ShowDocument(SpriteDocument.Create(_directory));

    private SpriteEditorDocument ShowDocument(SpriteDocument document)
    {
        var editor = new SpriteEditorDocument(document, SaveDocument);
    ...
    private bool SaveDocument(SpriteEditorDocument editor, bool saveAs)
    {
        try
        {
