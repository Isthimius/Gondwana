...
internal sealed class ImageViewport : ScrollableControl
{
    ...
    {
        if (SelectedRegion is not { } region || SelectedFrame is not { } frame) return;
        Rectangle bounds;
        try { bounds = FrameGeometry.Bounds(region, frame.X, frame.Y); }
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        base.OnMouseDown(e);
        Focus();
        if (Definition is null || e.Button != MouseButtons.Left) return;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        if (columns <= 0 || rows <= 0 || pitchX <= 0 || pitchY <= 0) return;
        ...
        {
            Rectangle bounds;
            try { bounds = FrameGeometry.Bounds(r, x, y); }
            catch (OverflowException) { return; }
